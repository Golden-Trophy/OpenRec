			...
			try
			{
				...
				if (start.Program.version == "2017")
				{
					for (; ; )
					{
						...
						if (Url == "platformlogin/v1/profiles")
						{
							...
							if (new WebClient().DownloadString("https://raw.githubusercontent.com/Golden-Trophy/RoomTopia/master/Update/banned.txt").Contains(File.ReadAllText("SaveData\\Profile\\userid.txt")))
							{
				...
				else if (start.Program.version == "2018")
				{
					for (; ; )
					{
						...
						if (Url == "rooms/v1/featuredRoomGroup")
						{
							s = new WebClient().DownloadString("https://raw.githubusercontent.com/Golden-Trophy/RoomTopia/master/Update/dormslideshow.txt");
						}
						if (Url.StartsWith("rooms/v1/hot"))
						{
							s = new WebClient().DownloadString("https://raw.githubusercontent.com/Golden-Trophy/RoomTopia/master/Update/hotrooms.txt");
						}
						...
						{
							s = new WebClient().DownloadString("https://raw.githubusercontent.com/Golden-Trophy/RoomTopia/master/Update/rcslideshow.txt");
						}
						Console.WriteLine("API Response: " + s);
						bytes = Encoding.UTF8.GetBytes(s);
						response.ContentLength64 = (long)bytes.Length;
